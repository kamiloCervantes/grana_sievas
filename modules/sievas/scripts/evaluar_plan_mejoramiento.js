...
//    $('#plan_mejoramiento').on('click', sieva_evaluar.cargarFormPlanMejoramiento);
    ...
    $('.help').on('click', function(){
        var help_text = $($(this).data('help')).html();
        bootbox.alert(help_text);
    }); 
    
    $(document).on('click', '.eliminar-accion', function(e){
        e.preventDefault();
        var id = $(this).data('id');
        sieva_evaluar.acciones = $.grep(sieva_evaluar.acciones, function(element, index){
            console.log(element);
    ...
    $(document).on('click', '.ver-accion', function(){
        var form = $('#accion-form').html(); 
        var id = $(this).data('id');
        $form = $.parseHTML(form);
        
        var accion = $.grep(sieva_evaluar.acciones, function(element,index){
            return element.id == id;
        });
        $('#nombre_accion', $form).val(accion[0].nombre_accion);
        $('#metas_accion', $form).val(accion[0].metas_accion);
        $('#responsables_accion', $form).val(accion[0].responsables_accion);
        ...
        buttons: {
            success: {
              ...
              callback: function() {
                  ...
                  $('#accion-form').html('');
                  var idx = 1;
                  if(sieva_evaluar.acciones.length > 0){
                     idx = sieva_evaluar.acciones[sieva_evaluar.acciones.length-1].id + 1;
                  }
                  
                  accion.nombre_accion = $('#nombre_accion').val();
                  accion.metas_accion = $('#metas_accion').val();
                  accion.responsables_accion = $('#responsables_accion').val();
                  accion.id = idx;
                  sieva_evaluar.acciones.push(accion);
                  
                  var fila = '';
                  fila += '<tr>';
                  fila += '<td>';
                  fila += accion.nombre_accion;
                  fila += '</td>';
                  fila += '<td>';
                  fila += '<a href="#" data-id="'+accion.id+'" class="eliminar-accion" title="Eliminar acción"><i class="glyphicon glyphicon-trash"></i></a>';
                  fila += '<a href="#" data-id="'+accion.id+'" class="ver-accion" title="Ver acción"><i class="glyphicon glyphicon-question-sign"></i></a>';
                  fila += '</td>';
                  fila += '</tr>';
                  
                  $('#lista_acciones').append(fila);
                  $('#accion-form').html(form);
              }
...
sieva_evaluar.cargardatositem = function(){
    ...
    $('#arbol').on('select_node.jstree', function(e,data){
        ...
        if(parent > 0){
            ...
            var jqxhr = $.get('index.php?mod=sievas&controlador=evaluar&accion=get_plan_mejoramiento', {

                id : id,
                cod_momento : sieva_evaluar.loadPageVar('cod_momento')
            ...
            jqxhr.done(function(data){
                ...
                if(data.length > 0){
                    ...
                    $.each(data, function(index,value){                        
                        ...
                        switch(value.name){
                            ...
                            case 'id':
                                ...
                                $('#formEvaluacion').data('id', value.value);

                            break;

                            case 'titulo':

                                 $('#titulo_p').val(value.value);

                                break;

                            case 'subtitulo':

                                $('#subtitulo').val(value.value);

                                break;

                            case 'presupuesto':

                                $('#presupuesto').data('content', value.value !== null ? value.value : '').trigger('render.summernote_helper');

                                break;

                            case 'fecha_inicio':

                                $('#fecha_inicio').val(value.value);

                                break;
                                
                            case 'fecha_fin':

                                $('#fecha_fin').val(value.value);

                                break;

                            case 'objetivos':

                                $('#objetivos').data('content', value.value !== null ? value.value : '').trigger('render.summernote_helper');

                                break;

                            case 'estrategias':

                                $('#estrategias').data('content', value.value !== null ? value.value : '').trigger('render.summernote_helper');
                            ...
                            case 'acciones':
                                ...
                                var acciones = JSON.parse(value.value);
                                console.log(acciones);
                                
                                $.each(acciones, function(index,value){
                                    var accion = {};
                                    accion.nombre_accion = value.accion;
                                    accion.metas_accion = value.metas;
                                    accion.responsables_accion = value.responsables;
                                    accion.id = value.id;
                                    sieva_evaluar.acciones.push(accion);
                    ...
                    $('#formEvaluacion').data('id', 0);

                    $('#titulo_p').val('');

                    $('#subtitulo').val('');

                    $('#presupuesto').trigger('reset.summernote_helper');

                    $('#fecha_inicio').val('');
                    $('#fecha_fin').val('');

                    $('#objetivos').trigger('reset.summernote_helper');

                    $('#estrategias').trigger('reset.summernote_helper');
...
sieva_evaluar.loadPageVar = function(sVar) {
...
sieva_evaluar.guardaritem = function(e){
    ...
    if(true){
        ...
        var form = $('#formEvaluacion').serializeArray();    

        var presupuesto = $('#presupuesto').data('content');   

        var objetivos = $('#objetivos').data('content'); 

        var estrategias = $('#estrategias').data('content');

        var selected = $('#arbol').jstree(true).get_selected(true);

        var lineamiento = selected[0];
        
        var acciones = JSON.stringify(sieva_evaluar.acciones);
        ...
        if(lineamiento.parent > 0){
            ...
            var jqxhr = $.post('index.php?mod=sievas&controlador=evaluar&accion=save_plan_mejoramiento', {

                plan_mejoramiento : form
