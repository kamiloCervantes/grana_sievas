var sieva_evaluar = {};
var socket = io('https://sievas.herokuapp.com');
    ...
    $('.anexos-list').on('recargar.archivos', function(){
        ...
        jqxhr.done(function(data){
            ...
            $.each(data, function(index,value){               
                ...
                if(value.color > 0){
                                    html += '<a href="index.php?mod=sievas&controlador=evaluaciones&accion=viewfile&id='+value.id+'" class="btn btn-xs btn-default" style="background-color: yellow" target="_blank">';

                }
                else{
                                    html += '<a href="index.php?mod=sievas&controlador=evaluaciones&accion=viewfile&id='+value.id+'" class="btn btn-xs btn-default" target="_blank">';
        ...
        if(parent > 0){            
            ...
            jqxhr.done(function(data){
                ...
                if(data.length > 0){                    
                    ...
                    var fortalezas = (data.fortalezas !== null && typeof data.fortalezas !== 'undefined')  ? data.fortalezas.replace(/%/g, "%25"):'';
                    var debilidades = (data.debilidades !== null  && typeof data.debilidades !== 'undefined' ) ? data.debilidades.replace(/%/g, "%25"):'';
                    var plan_mejoramiento = (data.plan_mejoramiento !== null  && typeof data.plan_mejoramiento !== 'undefined') ? data.plan_mejoramiento.replace(/%/g, "%25"):'';
                    $('#fortalezas').data('content', (data.fortalezas !== null  && typeof data.fortalezas !== 'undefined') ? decodeURIComponent(fortalezas) : '').trigger('render.summernote_helper');
                    $('#debilidades').data('content', (data.debilidades !== null  && typeof data.debilidades !== 'undefined') ? decodeURIComponent(debilidades) : '').trigger('render.summernote_helper');
                    $('#plan_mejoramiento').data('content', (data.plan_mejoramiento !== null  && typeof data.plan_mejoramiento !== 'undefined') ? decodeURIComponent(plan_mejoramiento) : '').trigger('render.summernote_helper');
                    //calificaciones
...
//                         $(e.target).parent().parent().parent().find('textarea.editor').code($.parseHTML(decodeURIComponent(data.traduccion)));
                         tinymce.activeEditor.setContent(data.traduccion);
                         var dato = tinymce.activeEditor.getContent();
                         console.log(dato);
                    }
                    else{
                        console.log("hola 3");
